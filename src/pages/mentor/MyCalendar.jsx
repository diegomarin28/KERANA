...
export default function MyCalendar() {
    ...
    const fetchDisponibilidad = async () => {
        ...
        if (error) {
            setError('Error cargando disponibilidad: ' + error);
        } else {
    ...
    const handleAddSlot = async (e) => {
        ...
        if (error) {
            setError('Error agregando horario: ' + error);
        } else {
    ...
    const handleDelete = async (id) => {
        ...
        if (error) {
            setError('Error eliminando horario: ' + error);
        } else {
    ...
    const handleToggle = async (id, activo) => {
        ...
        if (error) {
            setError('Error actualizando horario: ' + error);
        } else {
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
                <div>
                    ...
                    <p style={{ color: '#6b7280', margin: 0 }}>
                        Configura tus horarios disponibles para sesiones de mentoría
                    </p>
                </div>
                <button
            ...
            <div style={{
                ...
                {[1, 2, 3, 4, 5, 6, 0].map(dia => (
                    <div
                        ...
                        style={{
                            padding: 20,
                            borderBottom: dia !== 0 ? '1px solid #e5e7eb' : 'none'
                        }}
                    >
                        <h3 style={{
                            margin: '0 0 16px 0',
                        ...
                        {groupedByDay[dia] && groupedByDay[dia].length > 0 ? (
                                ...
                                {groupedByDay[dia].map(slot => (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            alignItems: 'center',
                                            padding: 16,
                                            background: slot.activo ? '#f0f9ff' : '#f9fafb',
                                            border: `1px solid ${slot.activo ? '#bfdbfe' : '#e5e7eb'}`,
                                            borderRadius: 8,
                                            opacity: slot.activo ? 1 : 0.6
                                        }}
                                    >
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                            <div style={{
                                                fontSize: 24,
                                                filter: slot.activo ? 'none' : 'grayscale(1)'
                                            }}>
                                                🕐
                                            </div>
                                            <div>
                                                <div style={{
                                                    fontWeight: 600,
                                                    color: slot.activo ? '#1e40af' : '#4b5563'
                                                }}>
                                                    {slot.hora_inicio} - {slot.hora_fin}
                                                </div>
                                                <div style={{
                                                    fontSize: 14,
                                                    color: slot.activo ? '#3b82f6' : '#6b7280'
                                                }}>
