...
import AuthModal_HacerResenia from './AuthModal_HacerResenia';
...
export default function MisReseniasModal({ open, onClose }) {
    const [ratings, setRatings] = useState([]);
    const [loading, setLoading] = useState(true);
    const [filteredRatings, setFilteredRatings] = useState([]);
    const [filters, setFilters] = useState({
        tipo: 'todos', // 'todos', 'profesor', 'mentor'
        ordenar: 'fecha-desc', // 'fecha-desc', 'fecha-asc', 'nombre-asc', 'calificacion-desc'
        materia: 'todas'
    });
    ...
    const loadRatings = async () => {
        ...
        try {
            ...
            console.log('✅ Reseñas cargadas:', data); // Para debug
    ...
    const handleEdit = (rating) => {
        ...
        // Cerrar "Mis Reseñas" primero
        onClose();

        // Abrir modal de edición después de un delay más largo
        setTimeout(() => {
    ...
    const handleSaveEdit = async (updatedData) => {
        try {
            ...
            if (error) {
                setToast({ message: 'Error al actualizar reseña', type: 'error' });
                return;
            }
    ...
    const goToEntity = (rating) => {
        if (rating.tipo === 'profesor') {
            navigate(`/profesores/${rating.ref_id}`); // ¿O es /professor?
        } else if (rating.tipo === 'mentor') {
            navigate(`/mentores/${rating.ref_id}`); // ¿O es /mentor?
        }
        onClose();
    };
            ...
            {showEditModal && editingRating && (
                <AuthModal_HacerResenia
                    ...
                    onClose={() => {
                        setShowEditModal(false);
                        setEditingRating(null);
                        // ✅ NO reabrir "Mis Reseñas" automáticamente
                    }}
                    ...
                    <div style={{
                        ...
                        {loading ? (
                            <div style={{ textAlign: 'center', padding: 40, color: '#64748b' }}>
                                ...
                                <p style={{ margin: 0, fontWeight: 600 }}>Cargando reseñas...</p>
                            </div>
                                        ...
                                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                                <div style={{
                                                    ...
                                                    height: 40,
                                                    borderRadius: 10,
                                                    background: rating.tipo === 'profesor'
                                                        ...
                                                        return (
                                                            <span
                                                                ...
                                                                style={{
                                                                    ...
                                                                    fontSize: 11,
                                                                    fontWeight: 600,
                                                                    background: tag.type === 'positive' ? '#d1fae5' : '#fef3c7',
                                                                    color: tag.type === 'positive' ? '#065f46' : '#78350f'
                                                                }}
                                                            >
                                                                {tag.label}
                                                            </span>
                                            ...
                                            {/* Fecha y workload */}
                                            <div style={{ display: 'flex', gap: 12, fontSize: 12, color: '#94a3b8', fontWeight: 500 }}>
                                                <span>
